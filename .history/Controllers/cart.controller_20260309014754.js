        ...
        try {
                ...
                for(let item of items){
                        if (user.cart_items.has(item.productname)){
                                ...
                                if(checkedquantity != `${productname} not found` && checkedquantity !=`Only ${stockAvailability} items in the stock from ${productinfo.name}`){
                                        user.cart_items.set(item.productname , currentquantity + item.quantity)
                                }
                                else if(checkedquantity == `${productname} not found`){
                                ...
                                if(checked != `${productname} not found` && checked !=`Only ${stockAvailability} items in the stock from ${productinfo.name}`){
                                      user.cart_items.set(item.productname , item.quantity)
                                }
                                else if(checked == `${productname} not found`){
