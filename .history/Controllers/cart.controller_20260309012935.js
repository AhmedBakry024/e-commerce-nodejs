...
const productExists = async (productname)=>{
        ...
        if(productdata == null){
                errors_product.push(`${productname} not found`)
                return null
        }
        else
                return productdata
...
const stockCheck = async (productname, productquantity) =>{
        ...
        if(productinfo){
                ...
                errors_quantity.push(`Only ${stockAvailability} items in the stock from ${productinfo.name}`)
        ...
        try {
                ...
                for(let item in items){
                        if (user.cart_items.has(item.productname)){
                                ...
                                if(checkedquantity != null || checkedquantity != "not available"){
                ...
                if (errors.length == 0)
                        ...
                        errors.forEach(element =>{
                                if(element != null){
                                        return res.status(200).json({message : element+" and the rest of the items added successfully"})
                                }
                                else{
                                        return res.status(401).json({message:"The product not found"})
                                }
                        })
                }
}            
